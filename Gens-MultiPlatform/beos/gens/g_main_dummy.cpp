...
int Set_Render(HWND hWnd, int Full, int Num, int Force, int w, int h, int flags)
{
	int Old_Rend, *Rend;
		...
		case -1:
			switch(Old_Rend)
			{
				...
				case 4:
					...
					puts("Render selected : 50% SCANLINE");
					break;
				...
				case 5:
					...
					puts("Render selected : 25% SCANLINE");
					break;

				case 6:
					if (Have_MMX) *Blit = Blit_Scanline_Int_MMX;
					else *Blit = Blit_Scanline_Int;
					puts("Render selected : INTERPOLATED SCANLINE");
					break;
				...
				case 7:
					...
					puts("Render selected : INTERPOLATED 50% SCANLINE");
					break;
				...
				case 8:
					...
					puts("Render selected : INTERPOLATED 25% SCANLINE");
					break;
				...
				case 9:
					...
					puts("Render selected : 2XSAI KREED'S ENGINE");
					break;
				...
				default:
					*Rend = 1;
					...
					puts("Render selected : DOUBLE(default)");
					break;
			}
			break;
	...
	End_DDraw();
	if (Sound_Initialised) Clear_Sound_Buffer();
	return(Init_DDraw(HWnd, w, h, flags));
}
...
int Set_Current_State(int hWnd, int Num)
{
	...
	{
		fclose(f);
		sprintf(Str_Tmp, "SLOT %d [OCCUPIED]", Current_State);
		Put_Info(Str_Tmp, 1500);
	}
	else
	{
		sprintf(Str_Tmp, "SLOT %d [EMPTY]", Current_State);
		Put_Info(Str_Tmp, 1500);
	}
	return 1;
}
	...
	if(num_joysticks)
	{
		if(poll_joystick() == 0)
		{
			for(int c = 0; c < joy[0].num_buttons; c++)
				input_set_joy_down(c, joy[0].button[c].b);
			
			input_set_joy_motion(joy[0].stick[0].axis[1].d1
				| joy[0].stick[0].axis[1].d2 << 1 | joy[0].stick[0].axis[0].d1 << 2
				| joy[0].stick[0].axis[0].d2 << 3, 0);
		}
	}
}
...
bool Init(int hInst, int nCmdShow)
{
	...
	strcpy(Gens_Path, "");
	strcpy(Language_Path, "language.dat");
	strcpy(Str_Tmp, "gens.cfg");
	//strcat(Gens_Path, "\\");
...
{
	int flags = GFX_AUTODETECT_WINDOWED, w = 320, h = 240;

	if(argc <= 1) {
		printf("Gens 2.12a (Genesis/32x emulator) by Stephane Dallongeville, BeOS port by Caz\n\n");
		printf("Gens Homepage: http://gens.consolemul.com/\n");
		printf("Using Allegro version: %s\n", allegro_id);
		printf("usage: %s <filename> [options]\n\n", argv[0]);
		printf("options:\n");
		printf("\t-s\tEnable Sound.\n");
		printf("\t-g\tGraphics Mode number. e.g -g 2\n");
		printf("\t-f\tFullscreen.\n");
		printf("\t-j\tJoystick.\n");
		printf("\t-v\tVsync.\n");
		return 1;
	...
	for(int i = 0; (i < argc || argv[i] != NULL); i++)
	{
		...
		if(!strcmp(argv[i], "-f"))
		{
			w = 640, h = 480, flags = GFX_AUTODETECT_FULLSCREEN;
		}
		if(!strcmp(argv[i], "-j")) { open_joystick(); use_joystick = true; }
		if(!strcmp(argv[i], "-v")) W_VSync = 1;
	}
	
	Set_Render(HWnd, Full_Screen, -1, 1, w, h, flags);
	set_close_button_callback(close_gens);
