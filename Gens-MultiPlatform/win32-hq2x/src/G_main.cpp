...
int Change_VGM_Sample_Accurate(HWND hWnd)
{
	if (VGM_SampleAccurate)
	{
		VGM_SampleAccurate = 0;
		MESSAGE_L("VGM frame accurate", "VGM frame accurate", 1000)
	}
	else
	{
		VGM_SampleAccurate = 1;
		MESSAGE_L("VGM sample accurate", "VGM sample accurate", 1000)
	}
...
BOOL Init(HINSTANCE hInst, int nCmdShow)
{
	...
	// Added by Maxim:
	Blit_HQ2x_InitLUTs(); // Initialise hq2x lookup tables
...
/*
	if (CC_Connect("CCGEN://Stef:gens@emu.consoleclassix.com/sonicthehedgehog2.gen", (char *) Rom_Data, CC_End_Callback))
	{
		MessageBox(NULL, "Error during connection", NULL, MB_OK);
	}
	else
		...
		case WM_COMMAND:
			...
			else switch(LOWORD(wParam))
			{
				...
				case ID_GRAPHICS_NEXT_RENDER:
					if ((Full_Screen) && (Render_FS < 10)) Set_Render(hWnd, 1, Render_FS + 1, false);
					else if ((!Full_Screen) && (Render_W < 10)) Set_Render(hWnd, 0, Render_W + 1, false);
					return 0;
				...
				case ID_SOUND_STARTVGMDUMP:
					if (VGM_Dumping) Stop_VGM_Dump();
					else Start_VGM_Dump();
					Build_Main_Menu();
					return 0;

				case ID_SOUND_VGMSAMPLEACCURATE:
					Change_VGM_Sample_Accurate(hWnd);
					return 0;
...
HMENU Build_Main_Menu(void)
{
	...
	MENU_L(MainMenu, 1, Flags, (UINT)Graphics, "Graphic", "", "&Graphics");
	MENU_L(MainMenu, 2, Flags, (UINT)CPU, "CPU", "", "&CPU");
	MENU_L(MainMenu, 3, Flags, (UINT)Sound, "Sound", "", "&Sound");
	MENU_L(MainMenu, 4, Flags, (UINT)Options, "Option", "", "&Options");
	...
	MENU_L(Files, i++, Flags, ID_FILES_LOADSTATEAS, "Load State as", "\tShift+F8", "Load S&tate...");
	...
	MENU_L(Graphics, 3, Flags, ID_GRAPHICS_COLOR_ADJUST, "Color", "", "&Colour Adjust...");
	...
	MENU_L(Graphics, 8, MF_BYPOSITION | MF_POPUP | MF_STRING, (UINT)GraphicsFrameSkip, "Frame Skip", "", "Frame S&kip");
	InsertMenu(Graphics, 9, MF_SEPARATOR, NULL, NULL);
	MENU_L(Graphics, 10, Flags | MF_UNCHECKED, ID_GRAPHICS_SHOT, "Screen Shot", "\tShift+Backspc", "Scr&eenshot");
	...
	if (Rend == 6)
	{
		MENU_L(GraphicsRender, i++, MF_BYPOSITION | MF_STRING | MF_CHECKED, ID_GRAPHICS_RENDER_INTESCANLINE, "Interpolated Scanline", "", "In&terpolated Scanline");
	}
	else
	{
		MENU_L(GraphicsRender, i++, MF_BYPOSITION | MF_STRING | MF_UNCHECKED, ID_GRAPHICS_RENDER_INTESCANLINE, "Interpolated Scanline", "", "In&terpolated Scanline");
	}
	...
	if (Have_MMX)
	{
		if (Rend == 7)
		{
			MENU_L(GraphicsRender, i++, MF_BYPOSITION | MF_STRING | MF_CHECKED, ID_GRAPHICS_RENDER_INT50SCANLIN, "Interpolated 50% Scanline", "", "Int&erpolated 50% Scanline");
		}
		else
		{
			MENU_L(GraphicsRender, i++, MF_BYPOSITION | MF_STRING | MF_UNCHECKED, ID_GRAPHICS_RENDER_INT50SCANLIN, "Interpolated 50% Scanline", "", "Int&erpolated 50% Scanline");
		}

		if (Rend == 8)
		{
			MENU_L(GraphicsRender, i++, MF_BYPOSITION | MF_CHECKED, ID_GRAPHICS_RENDER_INT25SCANLIN, "Interpolated 25% Scanline", "", "Inte&rpolated 25% Scanline");
		}
		else
		{
			MENU_L(GraphicsRender, i++, MF_BYPOSITION | MF_UNCHECKED, ID_GRAPHICS_RENDER_INT25SCANLIN, "Interpolated 25% Scanline", "", "Inte&rpolated 25% Scanline");
		}
		...
		if (Rend == 10)
		{
			MENU_L(GraphicsRender, i++, MF_BYPOSITION | MF_CHECKED, ID_GRAPHICS_RENDER_HQ2X, "hq2x", "", "&hq2x");
		}
		else
		{
			MENU_L(GraphicsRender, i++, MF_BYPOSITION | MF_UNCHECKED, ID_GRAPHICS_RENDER_HQ2X, "hq2x", "", "&hq2x");
		}
	}
	...
	if (VGM_Dumping)
	{
		MENU_L(Sound, i++, Flags, ID_SOUND_STARTVGMDUMP, "Stop VGM Dump", "", "Stop &VGM Dump");
	}
	else
	{
		MENU_L(Sound, i++, Flags, ID_SOUND_STARTVGMDUMP, "Start VGM Dump", "", "Start &VGM Dump");
	}

	if (VGM_SampleAccurate)
		InsertMenu(Sound, i++, Flags | MF_CHECKED, ID_SOUND_VGMSAMPLEACCURATE, "VGM sa&mple accurate");
	else
		InsertMenu(Sound, i++, Flags | MF_UNCHECKED, ID_SOUND_VGMSAMPLEACCURATE, "VGM sa&mple accurate");
