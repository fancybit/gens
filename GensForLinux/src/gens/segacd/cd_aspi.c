...
ASPI_Init (void)
{
  ...
  //By Ubi Start:
  FD_CDROM = open (CDROM_DEV, O_RDONLY);
  if (FD_CDROM >= 0)
    {
      Num_CD_Drive = 1;
      return 1;
  ...
  switch (HIBYTE (ASPI_Status))
    ...
    case SS_COMP:
      ...
      fprintf (debug_SCD_file,
	       "ASPI driver initialised : %d device(s) detected\n\n",
	       Num_CD_Drive);
#endif
      break;
...
#ifdef DEBUG_CD
      fprintf (debug_SCD_file,
	       "error : ASPI for Windows does not support this mode\n\n");
#endif
      return 0;
      break;
...
#ifdef DEBUG_CD
      fprintf (debug_SCD_file,
	       "error : an ASPI manager which does not support Windows is resident\n\n");
#endif
      return 0;
      break;
...
#ifdef DEBUG_CD
      fprintf (debug_SCD_file,
	       "error : ASPI for Windows is not initialized\n\n");
#endif
      return 0;
      break;
    }
...
int
ASPI_Get_Drive_Info (int dev, unsigned char *Inf)
{
  SRB_ExecSCSICmd s;
  CDD_Complete = 1;
  return -1;			//By Ubi:de momento...
  if ((dev + 1) > Num_CD_Drive)
    return 5;
...
int
ASPI_Test_Unit_Ready (int timeout)
{
  SRB_ExecSCSICmd s;
  int wait_time = 0;
  ...
  if ((CUR_DEV + 1) > Num_CD_Drive)
    return 5;
  CDD_Complete = 1;
  return 0;			//By Ubi, fd opened, ready...
...
int
ASPI_Set_CD_Speed (int rate, int wait)
{
  SRB_ExecSCSICmd s;
  //By Ubi Start
  int ret;
  if ((CUR_DEV + 1) > Num_CD_Drive)
    return 5;
  ret = ioctl (FD_CDROM, CDROM_SELECT_SPEED, (long) rate);
  if (ret < 0)
    return 2;
  CDD_Complete = 1;
  return 0;			//Ok!!
...
ASPI_Lock (int flock)
{
  ...
  if (ret < 0)
    return 0;			//what return with a error?
  CDD_Complete = 1;
  return 0;
...
//By Ubi Start
  ...
  if (ret < 0)
    return 0;			//What i return with a error?
  if (PostProc == NULL)
...
int
ASPI_Mechanism_State (int async,
		      int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
  SRB_ExecSCSICmd s_loc, *s;
...
int
ASPI_Play_CD_MSF (_msf * start, _msf * end, int async,
		  int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
  SRB_ExecSCSICmd s_loc, *s;
  //By Ubi Start:
  ...
  msf.cdmsf_min0 = start->M;
  msf.cdmsf_sec0 = start->S;
  msf.cdmsf_frame0 = start->F;
  msf.cdmsf_min0 = end->M;
  msf.cdmsf_sec0 = end->S;
  msf.cdmsf_frame0 = end->F;
  ret = ioctl (FD_CDROM, CDROMPLAYMSF, &msf);
  if (ret < 0)
    return 2;
  CDD_Complete = 1;
  return 0;
  ...
  if (PostProc == NULL)
    PostProc = ASPI_Play_CD_MSF_COMP;
		     ...
		     int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
  ...
  if (PostProc == NULL)
    PostProc = ASPI_Stop_Play_Scan_COMP;
...
int
ASPI_Pause_Resume (int resume, int async,
		   int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
  SRB_ExecSCSICmd s_loc, *s;
  ...
  if (PostProc == NULL)
    PostProc = ASPI_Pause_Resume_COMP;
		  ...
		  int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
  ...
  if (PostProc == NULL)
    PostProc = ASPI_Read_CD_LBA_COMP;
...
int
ASPI_Read_One_CD_LBA (int adr, unsigned char flag, unsigned char sub_chan,
		      int async,
		      int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
  SRB_ExecSCSICmd s_loc, *s;
  ...
  if (adr < 0)
    adr = 0;
  cdread.msf = LBA2MSF (adr);
  ret = ioctl (FD_CDROM, CDROMREADRAW, &cdread);
  memcpy (Buf_Read, cdread.buf + 16, 2048);
  //Falta el subchanel al final;
  CDD_Complete = 1;
  return 0;
  //By Ubi End.
...
int
ASPI_Read_CD_MSF (_msf * start, _msf * end, unsigned char sector,
		  unsigned char flag, unsigned char sub_chan, int async,
		  int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
  SRB_ExecSCSICmd s_loc, *s;
  ...
  if (PostProc == NULL)
    PostProc = ASPI_Read_CD_MSF_COMP;

  if (start == NULL)
    memset (&MSF_S, 0, sizeof (MSF_S));
  else
    memcpy (&MSF_S, start, sizeof (MSF_S));
  ...
  fprintf (debug_SCD_file,
	   "\n\nREAD CD MSF :\n Deb : %d:%d:%d   end %d:%d:%d\n\n", MSF_S.M,
	   MSF_S.S, MSF_S.F, MSF_E.M, MSF_E.S, MSF_E.F);
#endif
...
int
ASPI_Read_One_CD_MSF (_msf * start, unsigned char flag,
		      unsigned char sub_chan, int async,
		      int (*PostProc) (struct tagSRB32_ExecSCSICmd *))
{
  SRB_ExecSCSICmd s_loc, *s;
  _msf MSF_S, MSF_E;
...
int
ASPI_Read_TOC_LBA_COMP (SRB_ExecSCSICmd * s)
{
//    TOCTRACK *t;
...
int
ASPI_Read_TOC_MSF_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_Mechanism_State_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_Play_CD_MSF_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_Pause_Resume_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_Read_CD_LBA_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_Read_CD_MSF_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_Total_Lenght_CDD_c23_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
//              if (!(CDC.CTRL.B.B0 & 0x80)) CDD.Status |= SCD.Status_CDD;
      ...
      CDD.Minute =
	INT_TO_BCDW (SCD.TOC.
		     Tracks[SCD.TOC.Last_Track - SCD.TOC.First_Track +
			    1].MSF.M);
      CDD.Seconde =
	INT_TO_BCDW (SCD.TOC.
		     Tracks[SCD.TOC.Last_Track - SCD.TOC.First_Track +
...
int
ASPI_First_Last_Track_CDD_c24_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_Track_Adr_CDD_c25_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
//              if (!(CDC.CTRL.B.B0 & 0x80)) CDD.Status |= SCD.Status_CDD;
	...
	track_number = SCD.TOC.First_Track;
      ...
      CDD.Minute =
	INT_TO_BCDW (SCD.TOC.Tracks[track_number - SCD.TOC.First_Track].MSF.
		     M);
      CDD.Seconde =
	INT_TO_BCDW (SCD.TOC.Tracks[track_number - SCD.TOC.First_Track].MSF.
		     S);
...
int
ASPI_Play_CDD_c3_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_Pause_Play_CDD_c6_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_Resume_Play_CDD_c7_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
int
ASPI_CD_Init_CDD_COMP (SRB_ExecSCSICmd * s)
{
  //int i = 0;
...
int
ASPI_Def_CDD_COMP (SRB_ExecSCSICmd * s)
{
  ASPI_Command_Running = 0;
...
ASPI_Read_One_LBA_CDC (void)
{
  ...
  if (CDD.Control & 0x0100)	// DATA
      ...
      if (CDC.CTRL.B.B0 & 0x80)	// DECEN = decoding enable
	{
	  if (CDC.CTRL.B.B0 & 0x04)	// WRRQ : this bit enable write to buffer
	    {
	      memcpy (&CDC.Buffer[wr_adr + 4], cdread.buf + 16, 2048);
	      CDC.Buffer[wr_adr + 0] = INT_TO_BCDB (msf.minute);
	      CDC.Buffer[wr_adr + 1] = INT_TO_BCDB (msf.second);
	      CDC.Buffer[wr_adr + 2] = INT_TO_BCDB (msf.frame);
...
int
ASPI_Read_One_CDC_COMP (SRB_ExecSCSICmd * s)
{
...
int
ASPI_Read_One_CDC_Cache (void)
{
  if ((Sectors_In_Cache <= 0) && (SCD.Cur_LBA >= 0))
    return -1;
