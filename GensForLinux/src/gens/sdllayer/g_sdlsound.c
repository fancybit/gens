...
Init_Sound (void)
{
  ...
  if (CPU_Mode)
      ...
      for (i = 0; i < 312; i++)
	{
	  ...
	  Sound_Extrapol[i][1] =
	    (((Seg_Lenght * (i + 1)) / 312) - Sound_Extrapol[i][0]);
	}
      ...
      for (i = 0; i < 262; i++)
	{
	  ...
	  Sound_Extrapol[i][1] =
	    (((Seg_Lenght * (i + 1)) / 262) - Sound_Extrapol[i][0]);
	}
  ...
  spec.callback = proc;
  audiobuf =
    (unsigned char *) malloc ((spec.samples * spec.channels * 2 * 4) *
			      sizeof (short));

  spec.userdata = audiobuf;

  memset (audiobuf, 0,
	  (spec.samples * spec.channels * 2 * 4) * sizeof (short));
...
void
Write_Sound_Stereo (short *Dest, int lenght)
{
  int i, out_L, out_R;
  short *dest = Dest;
...
void
Dump_Sound_Stereo (short *Dest, int lenght)
{
  int i, out_L, out_R;
  short *dest = Dest;
...
void
Write_Sound_Mono (short *Dest, int lenght)
{
  int i, out;
  short *dest = Dest;
...
void
Dump_Sound_Mono (short *Dest, int lenght)
{
  int i, out;
  short *dest = Dest;
...
int
Write_Sound_Buffer (void *Dump_Buf)
{
  ...
  if (Sound_Stereo)
    {
      if (Have_MMX)
	Write_Sound_Stereo_MMX (Seg_L, Seg_R, (short *) pMsndOut, Seg_Lenght);
      else
	Write_Sound_Stereo ((short *) pMsndOut, Seg_Lenght);
    }
  else
    {
      if (Have_MMX)
	Write_Sound_Mono_MMX (Seg_L, Seg_R, (short *) pMsndOut, Seg_Lenght);
      else
	Write_Sound_Mono ((short *) pMsndOut, Seg_Lenght);
    }
  ...
  while (audio_len > 1024 * 2 * 2 * 4)
    nanosleep (&rqtp, NULL);	//SDL_Delay(1);
  return 1;
}


int
Clear_Sound_Buffer (void)
{
#if 0
  LPVOID lpvPtr1;
  DWORD dwBytes1;
  ...
  rval =
    lpDSBuffer->Lock (0, Seg_Lenght * Sound_Segs * Bytes_Per_Unit, &lpvPtr1,
		      &dwBytes1, NULL, NULL, 0);

  if (rval == DSERR_BUFFERLOST)
    {
      lpDSBuffer->Restore ();
      rval =
	lpDSBuffer->Lock (0, Seg_Lenght * Sound_Segs * Bytes_Per_Unit,
...
Update_WAV_Dump (void)
{
  ...
  if (WaveWriteFile
      (MMIOOut, lenght, &Buf_Tmp[0], &CkOut, &Writted, &MMIOInfoOut))
    {
      Put_Info ("Error in WAV dumping", 1000);
      return 0;
    }
